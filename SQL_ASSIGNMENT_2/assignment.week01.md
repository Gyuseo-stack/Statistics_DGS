...
EX) MYSQL, 오라클, SQL 서버, MariaDB

DBMS 유형 계층형, 망형, 관계형, 객체지향형
관계형 DBMS가 가장 많은 부분 차지

관계형 DBMS: RDBMS라고도 부름. 모든 데이터가 테이블로 조장
표준 SQL 학습 필요(여러 DBMS회사에서 표준 SQL을 준수하되, 각 제품의 특성을 반영한 SQL 사용)
